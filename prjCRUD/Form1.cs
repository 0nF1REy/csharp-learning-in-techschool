...
namespace prjCRUD
{
    ...
    {
        private readonly string conexaoBanco = "server=localhost;user id=root;database=bdcontato;password=aluno;";
        public frmPrincipal()
        {
            InitializeComponent();
            CarregarContatos();
        ...
        private void frmPrincipal_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                string conexaoBanco = $"server={servidor};" +
                    $"user id={usuario};database={bancoDados};" +
                    $"password={senha};";

                // Criar e testar a conexão
                using (MySqlConnection conexao = new MySqlConnection(conexaoBanco))
                {
                    conexao.Open();
                    conexao.Close();

                    lblTeste.Text = $"A conexão com o banco de dados: {bancoDados} foi realizada com sucesso!";
                }
            }
